...
from tracker import MultiTracker, cosine_distance, THE_BIGGEST_DISTANCE
...
class MultiCameraTracker:
    def __init__(self, reid_model,
        ...
        self.sct = MultiTracker(cam_id, reid_model, **sct_config)
    ...
    def process(self, frame, all_detections):
        ...
        self.sct.process(frame, all_detections,
                         screen_edge_objects, boundary_coord)
    ...
    def _merge_all(self):
            ...
            for j, mct_track in enumerate(self.tracks):
                if sct_track.cam_id == mct_track.cam_id and sct_track.id == mct_track.id:
                    ...
                    if dist <= self.global_match_thresh:
                        self.sct.tracks[i].f_clust.global_merge(
                            self.tracks[j].f_clust)
        ...
        while distance_matrix.shape[0] > 0 and distance_matrix.shape[1] > 0:
            ...
            if dist < self.global_match_thresh:
                ...
                stable_sct_track_indices = np.delete(
                    stable_sct_track_indices, j)
            else:
                break
    ...
    def _receeive_info(self):
        ...
        for i, cc_track in enumerate(self.mqtt_client.feats_temp):
            ...
            if cc_track[0] is not None:
                ...
                self.tracks.append(
                    MCT_track(id, cam_id, self.time, f_avg, f_clust))
...
class Messages:
    def __init__(self, clientname, topic,
        ...
        self.client.connect(broker_url, port=broker_port, keepalive=60)
    ...
    def deliver_pos(self, ID, cam_ID, device_ID, last_merge_dis):
        ...
        self.client.publish(
            topic=self.topic, payload=output, qos=0, retain=False)
